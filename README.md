...
6. Emits one station (`centroid-<node>`) per Portabo node and one platform (`<node>-<post>`) per post, exactly as `GetStations` lists them; posts on the exact same point are fanned out on a ~12 m circle as for Brno, and rail/ferry posts (90+) carry no platform number. Departures chunks stay keyed by the bare node and carry the platform each trip is expected at.
